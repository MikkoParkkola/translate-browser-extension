...
async function saveProviderConfig(cfg) {
  ...
  if (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.sync) {
    ...
    const toSave = {
      provider,
      providers,
      providerOrder: Array.from(new Set([provider, ...(cfg.providerOrder || [])])).filter(Boolean),
    ...
    toSave.endpoints = Object.entries(providers).reduce((acc, [id, info = {}]) => {
      if (info && info.apiEndpoint) {
        acc[id] = String(info.apiEndpoint);
      }
      return acc;
    }, {});
    if (primary.apiEndpoint) {
      toSave.endpoints[provider] = String(primary.apiEndpoint);
    }
