(function () {
  ...
  // Initialize logger
  const logger = self.qwenLogger?.create
  ...
  class BackgroundService {
    ...
    async _handleSmartTranslation(opts) {
      try {
        ...
        const strategy =
          opts.strategy || config.translation?.strategy || 'smart';
        const selectedProvider = this.configManager.selectProvider(
          config,
          strategy,
          textLength,
        );
        ...
        const apiKey = await this.storageManager.getProviderApiKey(
          selectedProvider.id,
        );
        if (!apiKey) {
          throw new Error(
            `No API key configured for provider: ${selectedProvider.name}`,
          );
        }
    ...
    async _getProvidersStatus() {
      try {
        ...
        for (const [providerId, provider] of Object.entries(
          ...
          const apiKey =
            await this.storageManager.getProviderApiKey(providerId);
    ...
    async _testProvider(providerId, apiKey) {
      try {
        ...
        const provider = this.configManager.getProviderConfig(
          {
            providers: {
              [providerId]: this.configManager.PROVIDER_DEFAULTS[providerId],
            },
          },
          providerId,
        );
  ...
  // Sanitize function for schema validation testing
  const _sanitizeHomeInitResponse = (payload) => {
    // Try to get schemas from global scope
    const homeInitResponseSchema = self.qwenSchemas?.homeInitResponseSchema;

    if (!homeInitResponseSchema) {
      logger.warn('Schema not available for sanitization');
      return payload;
    }

    try {
      return homeInitResponseSchema.parse(payload);
    } catch (error) {
      logger.warn('Schema validation failed:', error);
      return homeInitResponseSchema.parse({});
    }
  };

  // Expose for testing
  if (typeof module !== 'undefined') {
    module.exports = { backgroundService, _sanitizeHomeInitResponse };
  }
})();
