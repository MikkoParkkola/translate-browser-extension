...
class ProviderManager {
    ...
    async selectProvider(criteria) {
        ...
        if (capableProviders.length === 0) {
            console.warn('No capable providers found for criteria:', criteria);
            return null;
        }
        ...
        if (selectedProvider) {
            console.log(`Selected provider: ${selectedProvider.name} (score: ${providerScores[0].score.toFixed(3)})`);
        }
    ...
    async _loadProviderConfigurations() {
        ...
        console.log('Provider configurations will be loaded when provider classes are registered');
    }
    ...
    _startHealthMonitoring() {
        ...
        this.healthCheckTimer = setInterval(async () => {
            try {
                await this.checkHealth();
                console.log('Automated health check completed');
            } catch (error) {
                console.error('Automated health check failed:', error);
            }
        }, this.healthCheckInterval);

        console.log(`Health monitoring started (interval: ${this.healthCheckInterval / 1000}s)`);
    }
    ...
    updateSelectionWeights(newWeights) {
        ...
        console.log('Provider selection weights updated:', this.selectionWeights);
    }
    ...
    destroy() {
        ...
        console.log('Provider manager destroyed');
    }
}
