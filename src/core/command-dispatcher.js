...
class CommandDispatcher {
  constructor(logger, errorHandler) {
    ...
    this.ensureTestState = () => ({});
  }

  setFallbackHandlers(handlers = {}, getState = () => ({})) {
    this.fallbackHandlers = handlers || {};
    if (typeof getState === 'function') {
      this.ensureTestState = getState;
    }
  }
  ...
  async handleMessage(raw, sender, sendResponse) {
    try {
      ...
      if (!command) {
        const fallback = this.fallbackHandlers[msg.action];
        if (typeof fallback === 'function') {
          const state = this.ensureTestState ? this.ensureTestState() : {};
          Promise.resolve(fallback({ msg, sender, state }))
            .then(result => {
              if (result === undefined) {
            ...
            .catch(error => {
              this.logger?.error(`Fallback handler error for action ${msg.action}:`, error);
...
if (typeof module !== 'undefined' && module.exports) {
  ...
  if (typeof self !== 'undefined') {
    ...
    if (Array.isArray(self.__pendingCommandModules)) {
      self.__pendingCommandModules.forEach(fn => {
        try {
          fn(self.qwenCommandDispatcher);
        } catch (error) {
          try { console.error('Pending command registration failed', error); } catch {}
        }
      });
      self.__pendingCommandModules.length = 0;
    }
  } else if (typeof globalThis !== 'undefined') {
    globalThis.qwenCommandDispatcher = { CommandDispatcher, Command };
    if (Array.isArray(globalThis.__pendingCommandModules)) {
      globalThis.__pendingCommandModules.forEach(fn => {
        try {
          fn(globalThis.qwenCommandDispatcher);
        } catch (error) {
          try { console.error('Pending command registration failed', error); } catch {}
        }
      });
      globalThis.__pendingCommandModules.length = 0;
    }
  }
}
