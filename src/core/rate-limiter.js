...
class AdaptiveRateLimiter {
     ...
     */
    constructor() {
        this.logger = Logger.create('rate-limiter');
    ...
    async initialize() {
        try {
            ...
            this.logger.info('Adaptive rate limiter initialized for', this.providerLimits.size, 'providers');
        } catch (error) {
    ...
    _initializeProviderLimits() {
        ...
        this.logger.info('Provider limits initialized for', this.providerLimits.size, 'providers');
    }
    ...
    async recordUsage(providerId, metrics) {
        const usage = this.usageTracking.get(providerId);
        if (!usage) {
            this.logger.warn(`No usage tracking for provider: ${providerId}`);
            return;
        }
        ...
        this.logger.info(`Usage recorded for ${providerId}:`, {
        ...
        try {
            if (typeof chrome !== 'undefined' && chrome.storage) {
                ...
                if (result.rateLimiterUsage) {
                    ...
                    this.logger.info('Persisted usage data loaded');
                }
            }
        } catch (error) {
            this.logger.warn('Failed to load persisted usage data:', error);
        }
    }
    ...
    async _persistUsageData() {
        try {
            ...
            this.logger.warn('Failed to persist usage data:', error);
        }
    }
    ...
    _startMonitoring() {
        ...
        this.logger.info(`Rate limiter monitoring started (interval: ${this.monitoringInterval / 1000}s)`);
    }
    ...
    _performQuotaChecks() {
        this.providerLimits.forEach((limits, providerId) => {
            ...
            if (warnings.length > 0) {
                this.logger.warn(`Provider ${providerId} approaching limits:`, warnings.join(', '));
            }
        });
    }
    ...
    resetProviderUsage(providerId) {
        if (this.usageTracking.has(providerId)) {
            ...
            this._persistUsageData();
            this.logger.info(`Usage reset for provider: ${providerId}`);
        }
    }
