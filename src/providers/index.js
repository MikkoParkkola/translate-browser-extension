...
function initProviders() {
  ...
  // Load the hunyuan-local provider first (primary translation method)
  const localWasm = load('qwenProviderLocalWasm', './localWasm');
  if (localWasm && !baseGet('hunyuan-local')) {
    registerProvider('hunyuan-local', { ...localWasm, label: 'Local Model (Hunyuan-MT-7B)' });
    loadedProviders.add('hunyuan-local');
  }

  // Load the default Google Free provider as fallback (no API key required)
  const googleFree = load('qwenProviderGoogleFree', './googleFree');
  if (googleFree && !baseGet('google-free')) {
    registerProvider('google-free', googleFree.label ? googleFree : { ...googleFree, label: 'Google (public)' });
    loadedProviders.add('google-free');
  }

  // Load the default DashScope provider as fallback
...
function listProviders() {
  ...
  return Array.from(allProviders).map(name => {
    ...
    const defaultLabels = {
      ...
      'qwen': 'Qwen',
      'local-wasm': 'Local WASM',
      'hunyuan-local': 'Local Model (Hunyuan-MT-7B)'
    };
